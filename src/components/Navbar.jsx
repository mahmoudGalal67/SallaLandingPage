        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <svg className="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={isModalOpen ? "M6 18L18 6M6 6l12 12" : "M1 1h15M1 7h15M1 13h15"} />
            </svg>
          </button>
        </div>
        {/* <div
          ...
          id="navbar-sticky"
        > */}
                <div className={`items-center justify-between ${isModalOpen ? "block" : "hidden"} w-full md:flex md:w-auto md:order-1`} id="navbar-sticky">
            ...
            {["home", "education", "Solutions", "sectors", "prices"].map(
              (menu) => (
                <li
                  key={menu}
                  className={`relative ${
                    [ "education", "Solutions", "sectors"].includes(menu) ? "hover:rounded-t-lg hover:bg-white" : ""
                  }`}
 ...
 {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-30">
          <div className="fixed inset-0 z-40 bg-white p-4">
            <button className="text-gray-700" onClick={() => setIsModalOpen(false)}>X</button>
            <ul className="space-y-4 mt-4">
            <li><a href="#" className="block py-2 pl-3 pr-4 text-main_color font-bold  rounded">الرئيسية</a></li>
              ...
              <li>
                ...
                >
                  التعليم {activeMenu === "education" ? <FaMinus /> : <FaPlus />}
                </button>
                {activeMenu === "education" && (
                  <ul className="space-y-1">
                    {educationMenuItems.map((item, index) => (
                      <li key={index} className="pl-4 pr-4 py-2 bg-gray-50 border-b border-gray-200">
                        <a href="#" className="flex items-center text-gray-700">
                          <item.icon className="mr-2 text-2xl" />
                          <div>{item.name}<br /><small>{item.description}</small></div>
                        </a>
                      </li>
                    ))}
                  </ul>
              ...
              <li>
                ...
                >
                  الحلول {activeMenu === "solutions" ? <FaMinus /> : <FaPlus />}
                </button>
              ...
              <li>
                ...
                >
                  القطاعات {activeMenu === "sectors" ? <FaMinus /> : <FaPlus />}
                </button>
                {activeMenu === "sectors" && (
                  <ul className="space-y-1">
                    {sectorsMenuItems.map((item, index) => (
              ...
              <li><a href="#" className="block py-2 pl-3 pr-4 text-main_color font-bold  rounded">الأسعار</a></li>
          ...
          <Link to="/register">
          <button className="custom-navrbutton my-5 ">أنشئ متجرك مجاناً</button>
          </Link>
            </ul>
          </div>
        </div>
