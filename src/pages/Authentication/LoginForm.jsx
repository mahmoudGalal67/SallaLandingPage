...
const validationSchema = Yup.object({
  name: Yup.string().required('الاسم مطلوب').min(2, 'الاسم يجب أن يكون بين 2 و 100 حرف').max(100, 'الاسم يجب أن يكون بين 2 و 100 حرف'),
  email: Yup.string().email('البريد الإلكتروني غير صالح').required('البريد الإلكتروني مطلوب').min(5, 'البريد الإلكتروني يجب أن يكون بين 5 و 100 حرف').max(100, 'البريد الإلكتروني يجب أن يكون بين 5 و 100 حرف'),
  phone: Yup.string().required('رقم الجوال مطلوب').min(9, 'رقم الجوال يجب أن يكون بين 9 و 13 حرف').max(13, 'رقم الجوال يجب أن يكون بين 9 و 13 حرف'),
  password: Yup.string().required('كلمة المرور مطلوبة').min(8, 'كلمة المرور يجب أن تكون بين 8 و 16 حرف').max(16, 'كلمة المرور يجب أن تكون بين 8 و 16 حرف'),
});
      ...
      <div className="w-full max-w-md bg-white p-8">
        <form onSubmit={formik.handleSubmit}>
          ...
          <div dir="ltr" className="mb-4">
            <label dir='rtl' className="block text-gray-700 my-2">رقم الجوال</label>
            <PhoneInput
              name="phone"
            ...
            {formik.touched.phone && formik.errors.phone ? (
              <div className="text-red-500 text-sm text-right">{formik.errors.phone}</div>
            ) : null}
          </div>
          <div className="mb-4">
